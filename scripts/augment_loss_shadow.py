...
Section('training', 'Hyperparameters').params(
    ...
    in_shadow_model_save_path=Param(str, 'path to in model', required=True),
    out_shadow_model_save_path=Param(str, 'path to out model', required=True),

)

Section('data', 'data related stuff').params(
    augment_dataset=Param(str, 'file to store augmented datasets', required=True),
    in_shadow_observations=Param(str, 'Path to save in_set model logits', required=True),
    out_shadow_observations=Param(str, 'Path to save out_set model logits', required=True),
...
if __name__ == "__main__":
    ...
    print(f"Loading model from {config['training.in_shadow_model_save_path']}")
    model_in.load_state_dict(ch.load(config['training.in_shadow_model_save_path']))

    # Load the out_set model
    model_out = construct_model()
    print(f"Loading model from {config['training.out_shadow_model_save_path']}")
    model_out.load_state_dict(ch.load(config['training.out_shadow_model_save_path']))
    ...
    evaluate(model_in, loaders, config['data.in_shadow_observations'])

    print("Evaluating out_set model on augmented dataset and saving logits:")
    evaluate(model_out, loaders, config['data.out_shadow_observations'])
